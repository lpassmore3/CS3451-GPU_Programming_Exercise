...
def setup():
    ...
    catTexture = loadImage("data/lily_and_laura.jpg")
...
def green_mountain():
    ...
    textureMode(NORMAL)
    delta = 5
    texdelta = float(delta) / 200.0
    u = 0.0
    v = 0.0
    for i in range(-100, 100, delta):
        nextU = u + texdelta
        for j in range(-100, 100, delta):
            nextI = i + delta
            nextJ = j + delta
            nextV = v + texdelta
            beginShape()
            ...
            vertex(i, j, 0, u, v)
            vertex( i, nextJ, 0, u, nextV)
            vertex( nextI, nextJ,  0, nextU, nextV)
            vertex(nextI, j,  0, nextU, v)
            endShape()
            v = nextV
        u = nextU
        v = 0.0
